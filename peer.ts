...
export default <T>(producer?: Producer<T>): Peer<T> => {
  ...
  const publishToAllExcept = (exception: Subscriber<T>) =>
    (message: T) =>
      publishTo(subscribers.filter((subscriber) => subscriber !== exception))(
        message,
      );
  ...
  // TODO: cache2reuse
  const publishTo = (targets: Subscriber<T>[]) =>
    (message: T) =>
      later(() => {
        targets.filter((target) => subscribers.includes(target)).forEach(
          (target) => {
            try {
              target(message, {
                subscribe,
